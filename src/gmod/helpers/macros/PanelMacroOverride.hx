...
class PanelMacroOverride {
	...
	public static function build():Array<Field> {
		...
		var extResult = PanelMacro.generateGmodSideExtern({
			target: cls,
			gmodParent: parentGmod,
			targetFields: fields,
			markExpose: shouldAdd
		});
		...
		fieldstor.fields.iter(f -> if (f.meta != null) f.meta.push({name: ":notUser", pos: Context.currentPos()}); else f.meta = [{name: ":notUser",
